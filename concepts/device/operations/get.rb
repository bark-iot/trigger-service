...
class Device
  class Get < Trailblazer::Operation
    ...
    def model!(options, params:, **)
      begin
        response = RestClient.get("http://lb/houses/#{params[:house_id]}/devices/#{params[:id]}", headers={'Authorization' => params[:authorization_header]})
    ...
    def log_success(options, params:, model:, **)
      LOGGER.info "[#{self.class}] Found device with params #{params.to_json}. House: #{options['model'].to_s}"
    end
